...
function renderBookItem($row) {
    return "
        <div class='book-item'>
            ...
            <a href='mod_book.php?id=" . urlencode($row['id']) . "'><button>Bearbeiten</button></a>
...
// Funktion zur löschung von Büchern
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    $delete_id = filter_var($_POST['delete_id'], FILTER_VALIDATE_INT);
    if ($delete_id) {
        $delete_stmt = $conn->prepare("DELETE FROM buecher WHERE id = ?");
        $delete_stmt->bind_param("i", $delete_id);
        $delete_stmt->execute();
        $delete_stmt->close();
        // Direkt weiterleiten ohne Ausgabe, bleibe auf aktueller Seite
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    } else {
        // Bei ungültigen IDs direkt weiterleiten, bleibe auf aktueller Seite
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit;
    }
}
...
<body>
    ...
    <main>
        ...
        <a href='add_book.php'><button class = "button">Hinzufügen</button></a>
        <div class="book-list">
